...
// Timer configuration structure for persistence
struct TimerConfig {
  bool enabled;
  uint8_t channel;
  uint16_t minutes;
};
TimerConfig timerConfigs[RELAY_COUNT];
uint8_t timerConfigCount = 0;
...
uint32_t blynkOperationCount = 0;
uint32_t blynkLastHourCount = 0;
uint32_t blynkHourlyHistory[24] = {0};
int blynkCurrentHour = -1;
...
// Update Blynk hourly stats
void updateBlynkStats() {
  ...
  if (blynkCurrentHour == -1) {
    blynkCurrentHour = hour;
    blynkLastHourCount = blynkOperationCount;
  } else if (hour != blynkCurrentHour) {
    // Store the count for the previous hour
    blynkHourlyHistory[blynkCurrentHour] = blynkOperationCount - blynkLastHourCount;
    blynkLastHourCount = blynkOperationCount;
    blynkCurrentHour = hour;
    markDashStateDirty();
  }
...
// Timer configuration persistence functions
void loadTimerConfigs() {
  preferences.begin("timers", true);
  timerConfigCount = preferences.getUChar("count", 0);
  if (timerConfigCount > RELAY_COUNT) timerConfigCount = RELAY_COUNT;
  
  for (uint8_t i = 0; i < timerConfigCount; i++) {
    String key = "t" + String(i);
    String data = preferences.getString(key.c_str(), "");
    if (data.length() > 0) {
      int colon = data.indexOf(':');
      if (colon > 0) {
        timerConfigs[i].channel = data.substring(0, colon).toInt();
        timerConfigs[i].minutes = data.substring(colon + 1).toInt();
        timerConfigs[i].enabled = true;
      }
    }
  }
  preferences.end();
...
void saveTimerConfigs() {
  preferences.begin("timers", false);
  preferences.putUChar("count", timerConfigCount);
  for (uint8_t i = 0; i < timerConfigCount; i++) {
    String key = "t" + String(i);
    String data = String(timerConfigs[i].channel) + ":" + String(timerConfigs[i].minutes);
    preferences.putString(key.c_str(), data);
  }
  preferences.end();
}

void addTimerConfig(uint8_t channel, uint16_t minutes) {
  if (timerConfigCount >= RELAY_COUNT) return;
  timerConfigs[timerConfigCount].channel = channel;
  timerConfigs[timerConfigCount].minutes = minutes;
  timerConfigs[timerConfigCount].enabled = true;
  timerConfigCount++;
  saveTimerConfigs();
}

void removeTimerConfig(uint8_t index) {
  if (index >= timerConfigCount) return;
  for (uint8_t i = index; i < timerConfigCount - 1; i++) {
    timerConfigs[i] = timerConfigs[i + 1];
  }
  timerConfigCount--;
  saveTimerConfigs();
}

void clearTimerConfigs() {
  timerConfigCount = 0;
  saveTimerConfigs();
}
...
void setupRelays() {
  ...
  eventCount = 0;
  eventNextWrite = 0;
  blynkOperationCount = 0;
  blynkLastHourCount = 0;
  blynkCurrentHour = -1;
  for (int i = 0; i < 24; i++) blynkHourlyHistory[i] = 0;
}
    ...
    <section id="page-dashboard" class="page">
      <div class="card">
        ...
        <div class="row">
          ...
          <button class="btn reset-btn" onclick="resetDashboard()"><i class="fa-solid fa-rotate"></i> Reset Dashboard</button>
        </div>
      </div>
      <div class="miniGrid">
        ...
        <div class="card chartCard">
          <div><b>Graph 4: Blynk Operations per Hour</b></div>
          <div class="tiny">Total Blynk Ops: <span id="blynkTotalOps">0</span></div>
          <div id="blynkGraph" class="tiny">Loading...</div>
        </div>
      </div>
      <div class="card">
      ...
      <div class="card">
        <h4>Appliance ON/OFF History (Last 7 Days)</h4>
...
let lastTimerRenderKey='';
let lastBlynkGraphKey='';
const optionsHtml=`)rawliteral";
  html += scheduleOptions;
  html += R"rawliteral(`;

// Save and restore page state using localStorage (only for UI preference)
...
function renderBlynkGraph(hourlyData, totalOps) {
  const key = JSON.stringify({h:hourlyData||[], t:totalOps||0});
  if (key === lastBlynkGraphKey) return;
  lastBlynkGraphKey = key;
  
  const box = document.getElementById('blynkGraph');
  document.getElementById('blynkTotalOps').textContent = totalOps || 0;
  
  if (!hourlyData || hourlyData.length === 0) {
    box.textContent = 'No Blynk operations yet';
    return;
  }
  
  const data = hourlyData.map(v => Number(v || 0));
  const max = Math.max(1, ...data);
  const w = 360;
  const h = 140;
  const padX = 25;
  ...
  const innerW = w - padX * 2;
  const innerH = h - padY * 2;
  const barWidth = Math.min(20, innerW / data.length - 2);
  const gap = 2;
  const totalWidth = data.length * (barWidth + gap);
  const startX = padX + (innerW - totalWidth) / 2;
  ...
  data.forEach((v, i) => {
    ...
    bars += `<rect x="${x}" y="${y}" width="${barWidth}" height="${barH}" fill="#22d3ee" rx="2">
      <animate attributeName="height" from="0" to="${barH}" dur="0.5s" fill="freeze"/>
      <animate attributeName="y" from="${padY+innerH}" to="${y}" dur="0.5s" fill="freeze"/>
    </rect>`;
    const label = i < 10 ? '0' + i + ':00' : i + ':00';
    labels += `<text x="${x + barWidth/2}" y="${h-2}" text-anchor="middle" font-size="7" fill="#94a3b8">${label}</text>`;
    // Add value on top of bar
    if (v > 0) {
      labels += `<text x="${x + barWidth/2}" y="${y - 3}" text-anchor="middle" font-size="7" fill="#22d3ee">${v}</text>`;
    }
  });
  
  box.innerHTML = `
    <svg viewBox="0 0 ${w} ${h}" width="100%" height="150" style="display:block;background:#0b1220;border:1px solid #334155;border-radius:10px;padding:6px">
      <line x1="${padX}" y1="${padY+innerH}" x2="${w-padX}" y2="${padY+innerH}" stroke="#475569" stroke-width="1"></line>
      ${bars}
      ${labels}
    </svg>
  `;
...
function renderHistoryTable(history){
  ...
  const key = JSON.stringify(filteredHistory);
  if(key===lastHistoryKey) return;
  lastHistoryKey=key;
  const body=document.getElementById('historyTableBody');
  if(!filteredHistory || !filteredHistory.length){
    body.innerHTML='<tr><td colspan="4">No history in last 7 days</td></tr>';
    return;
  }
  body.innerHTML=filteredHistory.map(h=>`<tr><td>${h.time||'-'}</td><td>${h.name||'-'}</td><td>${h.state||'-'}</td><td>${h.source||'-'}</td></tr>`).join('');
}

// Render timers from server data
function renderTimersFromServer(timers) {
  if (!timers) timers = [];
  const key = JSON.stringify(timers);
  if (key === lastTimerRenderKey) return;
  lastTimerRenderKey = key;
  
  const container = document.getElementById('holdRows');
  container.innerHTML = '';
  ...
  timers.forEach((timer) => {
    const ch = String(timer.ch || 0);
    const min = String(timer.minutes || 10);
    addHoldRow(ch, min);
  });
}

// Render schedules from server data
function renderSchedulesFromServer(schedules) {
  if (!schedules || !schedules.length) return;
  const key = JSON.stringify(schedules);
  if (key === lastScheduleRenderKey) return;
  lastScheduleRenderKey = key;
  ...
  schedules.forEach((s, idx) => {
    if (s.en) {
      hasEnabled = true;
      addScheduleRow(String(idx), s.on || '08:00', s.off || '22:00', s.en);
    }
  });
...
function addHoldRow(defaultCh='0', defaultMin='10'){
  ...
  row.innerHTML=`
    ...
    <div class='row'>
      ...
      <button class='btn2' onclick='removeTimerRow(this)'>Remove</button>
    </div>`;
...
function removeScheduleRow(btn){ 
  ...
  fetch(`/set-schedule?ch=${ch}&on=00:00&off=00:00&en=0`)
    .then(() => refreshStatus());
  row.remove();
}
...
async function startHoldRow(btn){
  ...
  try{
    ...
    if(j.ok){
      ...
      await fetch(`/save-timer?ch=${ch}&minutes=${encodeURIComponent(min)}`);
      refreshStatus();
...
async function refreshStatus(){
  try{
    ...
    const newSchedules=j.schedules||[];
    const newTimers=j.timers||[];
    const newHistory=j.history||[];
    const newBlynkHourly=j.blynk_hourly||[];
    const newBlynkTotal=j.blynk_total_ops||0;
    ...
    const schedulesChanged=(JSON.stringify(newSchedules)!==JSON.stringify(latestSchedules));
    const timersChanged=(JSON.stringify(newTimers)!==JSON.stringify(latestTimers));
    const historyChanged=(JSON.stringify(newHistory)!==JSON.stringify(latestHistory));
    ...
    latestSchedules=newSchedules;
    latestTimers=newTimers;
    ...
    renderBlynkGraph(newBlynkHourly, newBlynkTotal);
    if(historyChanged) renderHistoryTable(latestHistory);
    updateCountdownRows(j.hold_remaining_sec||[]);
    
    if (schedulesChanged || namesChanged) {
      renderSchedulesFromServer(latestSchedules);
    }
    if (timersChanged || namesChanged) {
      renderTimersFromServer(latestTimers);
    }
  }catch(e){}
}
...
async function resetDashboard(){
  if(confirm('Reset dashboard statistics? This will clear history and reset counters.')){
    await fetch('/reset-dashboard');
    refreshStatus();
  }
}

// Initialization - restore page state
function initializeApp() {
  const savedPage = restorePageState();
  showPage(savedPage);
...
void handleStatus() {
  ...
  for (uint8_t i = 0; i < timerConfigCount; i++) {
    if (i) json += ",";
    json += "{\"ch\":" + String(timerConfigs[i].channel) + ",\"minutes\":" + String(timerConfigs[i].minutes) + "}";
  }
  json += "],\"blynk_hourly\":[";
  for (int i = 0; i < 24; i++) {
    if (i) json += ",";
    json += String(blynkHourlyHistory[i]);
  }
  json += "],\"blynk_total_ops\":" + String(blynkOperationCount);
...
void handleSaveTimer() {
  ...
  bool found = false;
  for (uint8_t i = 0; i < timerConfigCount; i++) {
    if (timerConfigs[i].channel == ch && timerConfigs[i].minutes == minutes) {
      found = true;
      break;
    }
  }
  ...
  if (!found) {
    addTimerConfig((uint8_t)ch, (uint16_t)minutes);
  }
...
void setupServer() {
  ...
  server.on("/reset-dashboard", HTTP_GET, []() { if (!requireAuth(true)) return; handleResetDashboard(); });
  ...
  server.on("/save-timer", HTTP_GET, []() { if (!requireAuth(true)) return; handleSaveTimer(); });
  server.on("/remove-timer", HTTP_GET, []() { if (!requireAuth(true)) return; handleRemoveTimer(); });
