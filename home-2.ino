...
let latestToday=[];
let latestSchedules=[];
let latestTimers=[];  // Store timers from server
...
function renderTimersFromServer(timers) {
  ...
  if (timers.length === 0) {
    // Add one default empty timer row
    addHoldRow();
    return;
  }
...
function renderSchedulesFromServer(schedules) {
  ...
  // If no enabled schedules, add one default row
  if (!hasEnabled) {
    addScheduleRow();
  }
}
...
async function refreshStatus(){
  try{
    ...
    // Render schedules and timers from server
